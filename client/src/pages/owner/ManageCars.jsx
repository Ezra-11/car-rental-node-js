...
const ManageCars = () => {
  ...
  const toggleAvaiability = async(carId)=>{
    try {
            ...
            {cars.map((car, index)=>(
              <tr key={index} className='border-t border-borderColor'>
                ...
                <td className='p-3 max-md:hidden'>
                  <span className={`px-13 py-1 rounded-full text-xs ${car.isAvaliable ? 'bg-green-100 text-green-500' : 'bg-red-100 text-red-500'}`}>
                    {car.isAvaliable ? 'Available': 'Unavailable'}
                  </span>
                </td>

                <td className='flex items-center p-3'>
                  <img onClick={()=> toggleAvaiability(car._id)} src={car.isAvaliable ? assets.eye_close_icon : assets.eye_icon} alt="" className='cursor-pointer'/>
